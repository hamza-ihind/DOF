  ...
  const fetchInitialData = async () => {
    ...
    try {
      ...
      if (eventsData.success) {
        const targetEvent = eventsData.events.find((e: any) => String(e.id) === String(eventId));
        if (targetEvent) {
          setTitle(targetEvent.title || '');
          setSlug(targetEvent.slug || '');
          setCategory(targetEvent.category || 'School Refurbishment');
          setDate(targetEvent.date || new Date().toISOString().split('T')[0]);
          setLocation(targetEvent.location || 'Biougra Region');
          setExcerpt(targetEvent.excerpt || '');
          setContent(targetEvent.content || '');
          setImpactSummary(targetEvent.impactSummary || '');
          setImageUrl(targetEvent.imageUrl || '/images/school_renovation.jpg');
          setSelectedMemberIds(targetEvent.contributingMemberIds || []);
          setTimelineDays(targetEvent.timelineDays && targetEvent.timelineDays.length > 0 
            ? targetEvent.timelineDays 
            : [{
                dayNumber: 1,
                title: 'Day 1: Mission Activities',
                date: targetEvent.date || new Date().toISOString().split('T')[0],
                description: targetEvent.excerpt || '',
                images: [targetEvent.imageUrl || '/images/school_renovation.jpg']
              }]
          );
        } else {
    ...
    return (
      <div className="min-h-screen bg-slate-50 flex items-center justify-center text-slate-800">
        <div className="flex flex-col items-center gap-3">
          <div className="w-10 h-10 border-4 border-[#aa1c34] border-t-transparent rounded-full animate-spin" />
          <p className="text-slate-600 text-xs font-semibold">Loading event details...</p>
        </div>
      </div>
    );
  }
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-slate-200 pb-4">
          <div>
            ...
            <h1 className="text-2xl font-extrabold text-[#0a4f6c]">
              Edit Event: <span className="text-[#aa1c34]">{title}</span>
            </h1>
            <p className="text-xs text-slate-500 font-medium">Update event details, contributing members, and multi-day timeline.</p>
          </div>
          ...
          <div className="bg-white rounded-2xl border border-slate-200 shadow-sm p-6 space-y-4">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div>
                ...
                <input
                  ...
                  placeholder="e.g. School Refurbishment & Supplies Mission"
                  className="w-full bg-slate-50 border border-slate-300 rounded-lg px-3.5 py-2.5 text-xs text-slate-900 focus:outline-none focus:border-[#0a4f6c] focus:bg-white"
                />
              </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              ...
              <div>
                ...
                <input
                  ...
                  placeholder="e.g. 120 Students Equipped"
                  className="w-full bg-slate-50 border border-slate-300 rounded-lg px-3.5 py-2.5 text-xs text-slate-900 focus:outline-none focus:border-[#0a4f6c] focus:bg-white"
                />
              </div>
            </div>
            ...
            <div>
              ...
              <textarea
                ...
                className="w-full bg-slate-50 border border-slate-300 rounded-lg px-3.5 py-2 text-xs text-slate-900 focus:outline-none focus:border-[#0a4f6c] focus:bg-white"
              />
            </div>
